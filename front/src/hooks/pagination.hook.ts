...
const usePagination = () => {
  ...
  // description: 한 섹션에 표시 될 페이지 리스트 상태  //
  ...
  const onPreviousClickHandler = () => {
    ...
    //   setCurrentPage((currentSection - 2) * PAGE_BY_SECTION + 1);
    // }
  ...
  const onNextClickHandler = () => {
    ...
    //   setCurrentSection(currentSection + 1);
    //   setCurrentPage(currentSection * PAGE_BY_SECTION + 1);
    // }
  ...
  const changeSection = (boardCount: number, countByPage: number) => {
    const {section, totalPageCount, maxPage, minPage} = getPagination(boardCount, currentSection, countByPage);
