...
import { AUTH_PATH, BOARD_WRITE_PATH, COUNT_BY_PAGE, MAIN_PATH, USER_EMAIL_PATH_VARIABLE, USER_PAGE_PATH, WRITE_PATH } from 'src/constants';
...
export default function UserPage() {
  ...
  //          component          //
  //  description: 유저페이지 하단  //
  const MyPageBottom = () => {

  //          state          //
  //  description: 페이지네이션과 관련된 상태 및 함수  //
  const { totalPage, currentPage, currentSection, changeSection,
          onPageClickHandler, onPreviousClickHandler, onNextClickHandler } = usePagination();
  ...
  //  description: 현재 페이지에서 보여 줄 게시물 리스트 상태 //
  ...
  const getPageBoardList = (boardList: BoardListResponseDto[]) => {
    ...
    const lateIndex = boardList.length > COUNT_BY_PAGE * currentPage ?
     COUNT_BY_PAGE * currentPage : boardList.length;
    const pageBoardList = boardList.slice(startIndex, lateIndex);
  ...
  const getUserResponseHandler = (responseBody: GetUserResponseDto | ResponseDto) => {
    ...
    const { email, nickname, profileImageUrl } = responseBody as GetUserResponseDto;
  }
  ...
  //          effect          //
  //  description: 유저 이메일이 바뀔 때마다 유저 정보 및 게시물 리스트 가져오기 //
  useEffect(() => {
    if (!userEmail) {
    ...
    getUserRequest(userEmail).then(getUserResponseHandler);
    getUserBoardListRequest(userEmail).then(getUserBoardListResponseHandler);

  },[userEmail])

  //  description: 현재 페이지가 바뀔 때 마다 유저페이지 게시물 분류  //
  useEffect(() => {
  ...
  return (
      <div className='my-page-bottom'>
        ...
        { boardCount !== 0 && (
          ...
          onPageClickHandler={onPageClickHandler} onPreviousClickHandler={onPreviousClickHandler} onNextClickHandler={onNextClickHandler} />
        ) }
      </div>
    );
  }
