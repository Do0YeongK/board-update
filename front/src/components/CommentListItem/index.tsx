...
export default function CommentListItem({ item } : Props) {
  ...
  const { profileImageUrl, nickname, writeDatetime, contents } = item;

  //          function            //
  // description: 현재시간과 작성시간이 차이 함수 //
  const getTimeGap = () => {
    ...
    let result = '';
    if (gap > 60 * 60) result = `${Math.floor(gap / 60 * 60)}시간 전`;
    if (gap < 60 * 60) result = `${Math.floor(gap / 60)}분 전`;
